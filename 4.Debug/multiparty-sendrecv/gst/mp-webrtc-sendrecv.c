...
static void send_room_peer_sdp(GstWebRTCSessionDescription *desc, const gchar *peer_id) {
  JsonObject *msg, *sdp;
...
static void on_offer_created(GstPromise *promise, const gchar *peer_id) {
  GstElement *webrtc;
...
static void remove_peer_from_pipeline(const gchar *peer_id) {
  gchar *qname;
...
static void add_peer_to_pipeline(const gchar *peer_id, gboolean offer) {
  int ret;
  gchar *tmp;
...
static void call_peer(const gchar *peer_id) {
  add_peer_to_pipeline(peer_id, TRUE);
}

static void incoming_call_from_peer(const gchar *peer_id) {
...
static gboolean join_room_on_server(void) {
  gchar *msg;
  if (soup_websocket_connection_get_state(ws_conn) != SOUP_WEBSOCKET_STATE_OPEN)
    return FALSE;
  if (!room_id)
...
static void do_join_room(const gchar *text) {
  ...
  if (app_state != ROOM_JOINING) {
    cleanup_and_quit_loop("ERROR: Received ROOM_OK when not calling", ROOM_JOIN_ERROR);
    return;
  }
...
static void on_answer_created(GstPromise *promise, const gchar *peer_id) {
  GstElement *webrtc;
...
static void handle_sdp_offer(const gchar *peer_id, const gchar *text) {
  ...
  promise = gst_promise_new_with_change_func((GstPromiseChangeFunc)on_answer_created, (gpointer)peer_id, NULL);
...
static gboolean handle_peer_message(const gchar *peer_id, const gchar *msg) {
  JsonNode *root;
...
static void connect_to_websocket_server_async(void) {
  ...
  session = soup_session_new_with_options(
    SOUP_SESSION_SSL_STRICT, strict_ssl, SOUP_SESSION_SSL_USE_SYSTEM_CA_FILE, TRUE,
...
static gboolean check_plugins(void) {
  ...
  const gchar *needed[] = {"opus", "nice", "webrtc", "dtls", "srtp", "rtpmanager", "audiotestsrc", NULL};
...
int main(int argc, char *argv[]) {
  ...
  if (!local_id)
    local_id = g_strdup_printf("%s-%i", g_get_user_name(), g_random_int_range(10, 10000));
